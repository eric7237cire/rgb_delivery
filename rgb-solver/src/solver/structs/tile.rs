...
impl TileEnum {
    pub fn get_van(&self) -> Option<&Van> {
        match self {
            TileRoad(Road { dynamic_attrs, .. }) | TileBridge(Bridge { dynamic_attrs, .. }) => {
                dynamic_attrs.van_snapshot.as_ref()
            }
    ...
    pub fn set_van(&mut self, van: &Van) {
        match self {
            TileRoad(Road { dynamic_attrs, .. }) | TileBridge(Bridge { dynamic_attrs, .. }) => {
                dynamic_attrs.van_snapshot = Some(van.clone())
            }
            Empty | TileWarehouse(..) => {}
        }
    }
    ...
    pub fn get_connection_mask(&self) -> Option<u8> {
        match self {
            TileRoad(Road { static_attrs, .. }) | TileBridge(Bridge { static_attrs, .. }) => {
    ...
    pub fn reset(&mut self) {
        match self {
            TileRoad(road) => {
                //comment if using ui forced moves
